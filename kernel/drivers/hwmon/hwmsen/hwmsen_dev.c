...
//Gionee wanght 2012-07-17 modify it for check device begin
#ifdef GN_MTK_BSP
#include <linux/gn_device_check.h>
extern int gn_set_device_info(struct gn_device_info gn_dev_info);
#endif
//Gionee wanght 2012-07-17 modify it for check device end
...
//Gionee mali 2012-05-29 modify for P-sensor MTK_compability policy begin
#define MAX_CHOOSE_P_NUM 5
//Gionee mali 2012-05-29 modify for P-sensor MTK_compability policy end

//Gionee yanggy 2012-03-23 modify for MTK_compability policy start
static char gsensor_name[25];
static char msensor_name[25];
//Gionee yanggy 2012-03-23 modify for MTK_compability policy end
//Gionee mali 2012-05-29 modify for P-sensor MTK_compability policy begin
...
//Gionee mali 2012-05-29 modify for MTK_compability policy begin
static struct sensor_init_info* psensor_init_list[MAX_CHOOSE_P_NUM]= {0};
//Gionee mali 2012-05-29 modify for MTK_compability policy end
/*----------------------------------------------------------------------------*/
struct dev_context {
    int		polling_running;
    struct mutex lock;
...
static int msensor_probe(struct platform_device *pdev) 
{
	...
	//Gionee wanght 2012-07-17 modify it for check device begin
	#ifdef GN_MTK_BSP
	struct gn_device_info gn_mydev_info_msensor;
	gn_mydev_info_msensor.gn_dev_type = GN_DEVICE_TYPE_MAGNETIC_FIELD ;
	#endif
	//Gionee wanght 2012-07-17 modify it for check device end
	*/
	...
	//Gionee wanght 2012-07-17 modify it for check device begin
	#ifdef GN_MTK_BSP
		   strcpy(gn_mydev_info_msensor.name, msensor_name);
		   gn_set_device_info(gn_mydev_info_msensor);
	#endif
	//Gionee wanght 2012-07-17 modify it for check device end
	*/
		   break;
	...
	if(i == MAX_CHOOSE_M_NUM)
	{
	   HWM_LOG(" msensor probe fail\n");
	}
...
//Gionee wanght 2012-07-17 modify it for check device begin
#ifdef GN_MTK_BSP
	struct gn_device_info gn_mydev_info_gsensor;
	gn_mydev_info_gsensor.gn_dev_type = GN_DEVICE_TYPE_ACCELEROMETER;
#endif
//Gionee wanght 2012-07-17 modify it for check device end
...
//Gionee wanght 2012-07-17 modify it for check device begin
#ifdef GN_MTK_BSP
		   strcpy(gn_mydev_info_gsensor.name, gsensor_name);
		   gn_set_device_info(gn_mydev_info_gsensor);
#endif
//Gionee wanght 2012-07-17 modify it for check device end
...
//Gionee yanggy 2012-03-30 add for ICS_KERNEL_exit begin
int hwmsen_gsensor_del(struct sensor_init_info* obj)
{
    int err = 0;
    int i = 0;
    HWM_FUN(f);
    for(i = 0;i < MAX_CHOOSE_G_NUM; i++ )
    {
        ...
        {
            HWM_ERR("No gsensor have been added!");
            return err;
        }
        else {
            kfree(gsensor_init_list[i]);
            return err;
        }
    }
}
//Gionee yanggy 2012-03-30 add for ICS_KERNEL_exit end 
...
int hwmsen_psensor_del(struct sensor_init_info *obj)
{
    int err = 0;
    int i = 0;
    HWM_FUN(f);
    for(i = 0;i < MAX_CHOOSE_P_NUM; i++ )
    {
        ...
        {
            HWM_ERR("No psensor have been added!");
            return err;
        }
        else {
            kfree(psensor_init_list[i]);
            return err;
        }
    }
}
