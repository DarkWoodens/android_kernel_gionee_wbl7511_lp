...
static void DSI_Config_VDO_Timing(void)
{
	...
	DSI_CHECK_RET(DSI_ConfigVDOTiming(lcm_params->dsi.vertical_sync_active,
                                      ...
                                      rgb_byte,
                                      (horizontal_sync_active_byte-6),
                                      (horizontal_backporch_byte-6),
                                      (horizontal_frontporch_byte-6)));
...
void init_dsi(BOOL isDsiPoweredOn)
{
	...
	if(lcm_params->dsi.mode != CMD_MODE)
	{
        ...
        DSI_CHECK_RET(DSI_PS_Control(lcm_params->dsi.PS, lcm_params->dsi.word_count));
    }
